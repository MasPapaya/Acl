...
class PermissionsController extends AclAppController {
	...
	public function admin_add() {
		if ($this->request->is('post') || $this->request->is('put')) {
			...
			$this->Session->setFlash(__d('acl', 'The Perm no has been saved.'), 'flash/error');
		}
	...
	public function admin_edit($id = NULL) {
		if ($id != NULL) {
			...
			if ($this->request->is('post') || $this->request->is('put')) {
				...
				$this->Session->setFlash(__d('acl', 'The Perm has been saved.'), 'flash/error');
				$this->redirect(array('action' => 'index'));
			}
			$aros = $this->ManagedAro->generateTreeList(NULL, NULL, NULL, '- ', NULL);
			$acos = $this->ManagedAco->generateTreeList(NULL, NULL, NULL, '- ', NULL);
			...
			$this->Session->setFlash(__d('acl', 'Perm id not set.'), 'flash/warning');
			$this->redirect(array('action' => 'index'));
		}
	}
	...
	public function admin_delete($id = NULL) {
		if ($id != NULL) {
			if ($this->request->is('post')) {
				$this->Perm->delete($id);
				$this->Session->setFlash(__d('acl', 'The Perm has been deleted'), 'flash/success');
			}
		}
		$this->redirect(array('action' => 'index'));
	}
